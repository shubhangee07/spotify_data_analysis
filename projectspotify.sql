...
select distinct artist from cleaned_dataset where stream>(select avg(stream) from cleaned_dataset) ;/*avg stream=455935814 ,artists having songs streamed more then avg stream*/
select  count(distinct artist)as artist_count from cleaned_dataset where stream>(select avg(stream) from cleaned_dataset) ;/*35=artists with streams greater then avg*/
select  avg(stream) as offvid from cleaned_dataset  where official_video="true";/*avg stream of tacks having official vid=480360036.0032258*/
 select avg(stream) as nooffvid from cleaned_dataset where  official_video="false";/*avg stream of tacks having official vid=298196047.75*/
